...
linexe_wrapper_stochastic <- function(flood_vect, phid, phif) {
  ...
  linexe <- function(y_true, y_pred){ 
    ...
    mod_loss <- K$mean(10^6*(exp_loss-lin_loss)) # added a beta in the beginning
...
library(keras)
# library(reticulate) # seems like this resolved error 2 in installing a keras environment on the watershed computer
  ...
  for (k in 1:(length(unique(data$CDEC_ID)))){
    ...
    if(losstype=="linexe"){
      nnmodel %>% # try "rmsprop", "adam", "sgd", "nadam" 
        compile(optimizer="adam", loss=linexe_wrapper_stochastic(flood_vect=trainsetpvs_scaled2[, "FLOOD"], phid=-10^-6, phif=10^-6), metrics=c("mae"))
      nnmodel %>% 
...
nn_agg_logo_mse_results <- nncvloss(df_lm_agg, "mse")
nn_agg_logo_mae_results <- nncvloss(df_lm_agg, "mae")
nn_agg_logo_mape_results <- nncvloss(df_lm_agg, "mape")
nn_agg_logo_mlae_results <- nncvloss(df_lm_agg, "mlae")
nn_agg_logo_mslae_results <- nncvloss(df_lm_agg, "mslae")
nn_agg_logo_wlse_results <- nncvloss(df_lm_agg, "wlse")

nn_agg_logo_linexe_results <- nncvloss(df_lm_agg, "linexe")
