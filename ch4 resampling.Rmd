...
glmcv <- function(data, groupingstyle, datatype, ...){ 
  # for resubstitution, where testing data and training data is the same
  if(groupingstyle=="resub"){
    linearmodel <- glm(FLOW~. , data=data[,c(15,18:ncol(data))], ...)
    predictions <- predict(linearmodel, data, type='response')
    results <- cbind(obs=data$FLOW, pred=predictions)
    modgof <- gof(as.data.frame(results)$pred, as.data.frame(results)$obs)
  }
...
library(statmod)
# glmtwd_agg_resub_results <- glmcv(df_lm_agg, "resub", "agg", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_agg_2fold_results <- glmcv(df_lm_agg, 2L, "agg", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_agg_5fold_results <- glmcv(df_lm_agg, 5L, "agg", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_agg_10fold_results <- glmcv(df_lm_agg, 10L, "agg", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_agg_logo_results <- glmcv(df_lm_agg, "logo", "agg", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_agg_lmgo_results <- glmcv(df_lm_agg, "lmgo", "agg", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# 
# glmtwd_inc_resub_results <- glmcv(df_lm_inc, "resub", "inc", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_inc_2fold_results <- glmcv(df_lm_inc, 2L, "inc", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_inc_5fold_results <- glmcv(df_lm_inc, 5L, "inc", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_inc_10fold_results <- glmcv(df_lm_inc, 10L, "inc", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_inc_logo_results <- glmcv(df_lm_inc, "logo", "inc", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# glmtwd_inc_lmgo_results <- glmcv(df_lm_inc, "lmgo", "inc", family=tweedie(var.power=1.1, link.power=0), maxit=1000)
# 
...
dim(as.data.frame(do.call("rbind", rf_agg_lmgo_results$results)))[[1]] == dim(results_agg)[[1]]
dim(as.data.frame(do.call("rbind", rf_inc_lmgo_results$results)))[[1]] == dim(results_inc)[[1]]
