...
png('ch4 resampling/outputdata/rplot41_gof_KGE.png', width=6.5, height=4, units="in", pointsize=8, res=1200)
  ...
  dotchart(goftablet$KGE, labels=var_labels[match(rownames(goftablet), names(var_labels))], xlab="Kling-Gupta Efficiency (-)", groups=goftablet$GROUPS, pch=19, xlim=c(min(goftablet$KGE), 1), color= cbpblack[5:8][goftablet$MODELTYPE])
...
# log scale
densityplot3 <- function(resultsdf, modeltype, legendpos){
  resubfit <- paste0(toupper(modeltype), "_RESUBFIT")
  twofoldfit <- paste0(toupper(modeltype), "_2FOLDFIT")
  fivefoldfit <- paste0(toupper(modeltype), "_5FOLDFIT")
  tenfoldfit <- paste0(toupper(modeltype), "_10FOLDFIT")
  ...
  png(paste0('ch4 resampling/outputdata/rplot45_density_', modeltype, '.png'), width=3.25, height=2.85, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,3,3)+0.1, cex=1)
    plot(density(resultsdf$FLOW), main="", log='x')
    rug(jitter(resultsdf$FLOW))
    lines(density(resultsdf[, resubfit], na.rm = TRUE), col=cbpblack[2])
    lines(density(resultsdf[ , twofoldfit], na.rm = TRUE), col=cbpblack[3])
    lines(density(resultsdf[ , fivefoldfit], na.rm = TRUE), col=cbpblack[4])
...
densityplot3(results_agg, "lm", "topright") 
densityplot3(results_agg, "glm", "topright") 
densityplot3(results_agg, "rf", "topright") 
densityplot3(results_agg, "nn", "topright")

library(reshape2)
library(ggplot2)
library(ggthemes)
# densityplot4 <- function(resultsdf, modeltype){
#   resubfit <- paste0(toupper(modeltype), "_RESUBFIT")
#   twofoldfit <- paste0(toupper(modeltype), "_2FOLDFIT")
#   fivefoldfit <- paste0(toupper(modeltype), "_5FOLDFIT")
#   tenfoldfit <- paste0(toupper(modeltype), "_10FOLDFIT")
...
#   
#   png(paste0('ch4 resampling/outputdata/rplot46_density_', modeltype, '.png'), width=3.25, height=2.85, units="in", pointsize=8, res=1200)
#   ggplot(results_all_melted) +
#     geom_freqpoly(aes(x=value, y=..density.., color = variable), binwidth = 1e5) + 
#     xlim(0.001, 4e6) +
#     scale_color_manual(values=c(cbpblack, cbpgrey[1]))+
#     scale_y_continuous(trans="log10", name="density")+
#     labs(x ="Unimpaired Flow (AF), Binwidth=100,000", y = "Density or Frequency", color = "")+
#     theme_bw()
#   dev.off()
# }
# 
# densityplot4(results_agg, "lm") 
# densityplot4(results_agg, "glm") 
# densityplot4(results_agg, "rf") 
# densityplot4(results_agg, "nn") 
```
